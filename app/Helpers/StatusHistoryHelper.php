...
//                    $notify = '<b>'.$sender_name.'</b> tarafından <a href="/sale-detail/'.$sale->sale_id.'"><b>'.$short_code.'</b></a> numaralı siparişin durumu <b>"'.$new_status->name.'"</b> olarak güncellendi.';
                    $action_link = env('APP_URL').'/sale-detail/'.$sale->sale_id;
                    $notify_logo = env('APP_URL').$sale_owner->notification_logo;

                    if ($notify_setting->is_notification == 1){
                        $notify = '<a href="'.env('APP_URL').'/sale-detail/'.$sale->sale_id.'"><b>'.$short_code.'</b></a> '.$notify_setting->message;
